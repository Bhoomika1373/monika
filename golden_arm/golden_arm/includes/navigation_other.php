<header id="header" class="header d-flex align-items-center fixed-top other-header">
  <div class="container-fluid container-xl position-relative d-flex align-items-center justify-content-between">
    <a href="/golden_arm/home" class="logo d-flex align-items-center">
       <img src="assets/img/main-logo.png" alt="">
    </a>
    <nav id="navmenu" class="navmenu">
      <ul>
        <li><a href="/golden_arm/home" class="item-home"><i class="bi bi-house-fill"></i></a></li>
        <li><a href="/golden_arm/menu" class="item-menu">Menu</a></li>
        <li><a href="/golden_arm/pickup" class="item-pickup">Pickup</a></li>
        <li><a href="/golden_arm/reservation" class="item-reservation">Reservation</a></li>
        <li>
          <a href="javascript:void(0)" class="item-home" id="cart-icon">
            <i class="bi bi-cart-fill"></i>
            <span class="position-absolute top-0 start-100 translate-middle badge rounded-pill bg-danger" id="cart-counter">
              2
            </span>
          </a>
        </li>
      </ul>
    </nav>
  </div>
</header>
